...
BRIRConvolutionFilter::BRIRConvolutionFilter(wstring filename)
{
	this->filename = filename;
	filters = NULL;
}

BRIRConvolutionFilter::~BRIRConvolutionFilter()
{
	cleanup();
}

void BRIRConvolutionFilter::initialize()
{
	cleanup();
	...
	else if (abs(this->sampleRate - info.samplerate) > 1.0f)
	{
		LogF(L"Impulse response sample rate (%d Hz) does not match device sample rate (%f Hz)", info.samplerate, this->sampleRate);
	}
	else
	{
...
void BRIRConvolutionFilter::process(float** output, float** input, unsigned frameCount)
{
	if (filters == NULL){
		return;
	}
...
void BRIRConvolutionFilter::cleanup()
{
	if (filters != NULL)
	{
		for (unsigned i = 0; i < channelCount; i++)
